...
class Account:
    ...
    def __init__(self,pk=gen_private_key()):
        self.pk=pk
...
class BitcoinAccount(Account):
    ...
    def to_pub(self,key,compressed=True):
        public = coincurve.PublicKey.from_secret(self.pk)\
    ...
    def to_address(self,network_prefix=network_addrprefix,compressed=True):
        return to_P2PKH(network_prefix=network_prefix,compressed=compressed)
############## Ethereum tools ##############
...
#§§§#
"""
    def priv_to_wif(self,key,network_prefix=network_wifprefix,compressed=True):
        s1 = bytes([network_prefix]) + key 
        if(compressed):
            s1+= bytes([0x01]) # add compressed flag byte
        s2 = hash256(s1)
        s3 = hash256(s2)
    ...
    def gen_wif(self,compressed=True): 
        return priv_to_wif(key=self.pk,compressed=compressed)

    def wif_to_priv(self,wif,compressed=True):
        pkeychecked = b58decode(wif) # convert to base58
    ...
    def test_wif_checksum(self,wif,network_prefix=network_wifprefix):
        pkeychecked = b58decode(wif)
        checksum_to_test = pkeytested[-4:]
        pkey = pkeytested[:-4]
        s1 = hash256(pkey)
    ...
    def wif_uncompressed_to_compressed(self,wif,network_prefix=network_wifprefix):
        priv = wif_to_priv(wif,compressed=False)
        return priv_to_wif(priv,network_prefix=network_prefix,compressed=True)

    def wif_compressed_to_uncompressed(self,wif,network_prefix=network_wifprefix):
        priv = wif_to_priv(wif,compressed=True)
        return priv_to_wif(priv,network_prefix=network_prefix,compressed=False)

    def priv_to_pub(self,key,compressed=True):
        public = coincurve.PublicKey.from_secret(key).format(compressed=compressed)
        return public
    ...
    def pub_to_P2PKH(self,pub,network_prefix=network_addrprefix):
        encrypted_pub = bytes([network_prefix]) + hash160(hash256(pub))
        check = hash256(hash256(encrypted_pub))
        checksum = check[:4]
        address = encrypted_pub + checksum
